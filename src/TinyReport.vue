<template>
  <div class="tiny-paper-box">
    <div class="tiny-paper rd-f5" :style="{'width':report.paper.width + 'px','height':report.paper.height + 'px', fontSize:report.paper.fontsize + 'px'}">
      <div class="tiny-paper-content input-text" ref="report" @mousedown="onMouseDown" @mousemove="onMouseMove" @mouseup="onMouseUp" @dragover="onDragOver" @drop="onDrag">
            <report-base-item
              ...
              > 
                      ...
                      <TinyInputText :options="options" :model="model" :tabindex="item.tab + ''"  v-if="item.class == 'input-text'" v-model="report.items[key].data"/>
...
import TinyLabelText from './ReportComponents/TinyLabelText.vue'
import TinyLabelData from './ReportComponents/TinyLabelData.vue'
import TinyTop from './ReportComponents/TinyTop.vue'
import TinyInputText from './ReportComponents/TinyInputText.vue'
...
import TinySelectItem from './ReportComponents/TinyItemSelect.vue'



import Var from './TinyVariable'

import ReportBaseItem from './ReportComponents/index.vue'


export default {
  name: 'YmTinyReport',
  components:{ReportBaseItem, TinyRich, TinyImage, TinyQrcode,TinyEllipse, TinySelectItem, TinyRect, TinyLabelText, TinyLabelData, TinyTop, TinyInputText, TinyTextarea, TinySelectDate, TinySelectCascader},
  props: {
      readOnly: {
          type: Boolean,
          default: function() {
  ...
  watch:{
    report:{
      handler(newVal, oldVal){
        this.$emit('reportChange', JSON.stringify(newVal))
      },
      deep:true,
      immediate:false,
    },
  ...
  methods:{
    ...
    AddItemByType(report_item, x, y) {
      ...
      */
      ...
      if(type_name === "image") {
          this.addItem({id:new_id, class:type_name, friend_name, left:x,top:y,width:100,height:100, isActive:true, zindex:0, selectted:false, data: preset_data, ex_data}); 
      }
          ...
          this.addItem({id:new_id, class:type_name,friend_name, tab:def_tab, left:x,top:y,width:100,height:30, fontfamily,fontweight,fontcolor, align, fontsize,isActive:true, zindex:0, selectted:false, data: preset_data, ex_data}); 
      }
